    ...
    function _withdrawERC20(bool useRouter, uint256 amount, uint256 gasLimit, uint256 feePerGas) private {
        ...
        if (amount == 0) {
            ...
            // emit SentMessage from L2T1Messenger
            {
                ...
                emit SentMessage(
                    address(gateway), address(counterpartGateway), 0, 0, gasLimit, message, T1Constants.ETH_CHAIN_ID, keccak256(xDomainCalldata)
                );
            }
        ...
        admin.upgrade(
            ...
            address(
                new L2StandardERC20Gateway(
                    address(counterpartGateway), address(router), address(messenger), address(factory)
                )
            )
        );
    }
