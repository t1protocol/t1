...
contract L1WETHGatewayTest is L1GatewayTestBase {
    ...
    function skiptestDropMessageMocking() public {
        ...
        vm.expectRevert(ErrorNotInDropMessageContext.selector);
        ...
        vm.expectRevert("token not WETH");
    ...
    function testDropMessage(uint256 amount, address recipient, bytes memory dataToCall) public {
        ...
        gateway.depositERC20AndCall(address(l1weth), recipient, amount, dataToCall, DEFAULT_GAS_LIMIT);
        ...
        public
    {
        ...
        // l1 token not WETH
        vm.expectRevert("l1 token not WETH");
        mockMessenger.callTarget(
            address(gateway),
            abi.encodeWithSelector(
                gateway.finalizeWithdrawERC20.selector,
        ...
        // l2 token not WETH
        vm.expectRevert("l2 token not WETH");
        mockMessenger.callTarget(
            address(gateway),
            abi.encodeWithSelector(
                gateway.finalizeWithdrawERC20.selector,
        ...
        public
    {
        ...
        // deposit some WETH to L1T1Messenger
        gateway.depositERC20(address(l1weth), amount, DEFAULT_GAS_LIMIT);
        ...
        assertEq(false, l1Messenger.isL2MessageExecuted(keccak256(xDomainCalldata)));
    ...
    function testFinalizeWithdrawERC20(address sender, uint256 amount, bytes memory dataToCall) public {
        ...
        assertEq(true, l1Messenger.isL2MessageExecuted(keccak256(xDomainCalldata)));
    }
    ...
    function _depositERC20(bool useRouter, uint256 amount, uint256 gasLimit, uint256 feePerGas) private {
        ...
        if (amount == 0) {
            ...
            // token is not l1WETH
            vm.expectRevert("only WETH is allowed");
    ...
    function _deployGateway(address messenger) internal returns (L1WETHGateway _gateway) {
        ...
        admin.upgrade(
            ...
            address(
                new L1WETHGateway(
                    address(l1weth), address(l2weth), address(counterpartGateway), address(router), address(messenger)
                )
            )
        );
    }
