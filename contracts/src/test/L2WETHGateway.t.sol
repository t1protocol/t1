...
contract L2WETHGatewayTest is L2GatewayTestBase {
    ...
    function testWithdrawERC20(uint256 amount, uint256 gasLimit, uint256 feePerGas) public {
        ...
        public
    {
        ...
        // send some WETH to L2T1Messenger
        gateway.withdrawERC20(address(l2weth), amount, 21_000);
    ...
    function testFinalizeDepositERC20(address sender, uint256 amount, bytes memory dataToCall) public {
        MockGatewayRecipient recipient = new MockGatewayRecipient();

        amount = bound(amount, 1, l2weth.balanceOf(address(this)));

        // send some WETH to L1T1Messenger
        gateway.withdrawERC20(address(l2weth), amount, 21_000);
        ...
        if (amount == 0) {
            ...
            if (useRouter) {
                ...
                gateway.withdrawERC20AndCall{ value: feeToPay }(
                    address(l2weth), recipient, amount, dataToCall, gasLimit
                );
            }
        } else {
            // token is not l1WETH
    ...
    function _deployGateway(address messenger) internal returns (L2WETHGateway _gateway) {
        ...
        admin.upgrade(
            ...
            address(
                new L2WETHGateway(
                    address(l2weth), address(l1weth), address(counterpartGateway), address(router), address(messenger)
                )
            )
        );
    }
