...
contract T1Chain is OwnableUpgradeable, PausableUpgradeable, IT1Chain {
    ...
    function importGenesisBatch(bytes calldata _batchHeader, bytes32 _stateRoot) external {
        ...
        unchecked {
            uint256 sum = BatchHeaderV0Codec.getVersion(memPtr) + BatchHeaderV0Codec.getBatchIndex(memPtr)
                + BatchHeaderV0Codec.getL1MessagePopped(memPtr) + BatchHeaderV0Codec.getTotalL1MessagePopped(memPtr);
        ...
        whenNotPaused
    {
        ...
        if (1 <= _version && _version <= 2) {
            ...
            (_dataHash, _totalL1MessagesPoppedInBatch) =
                _commitChunksV1(_totalL1MessagesPoppedOverall, _chunks, _skippedL1MessageBitmap);
            assembly {
                batchPtr := mload(0x40)
            ...
            _batchHash = BatchHeaderV0Codec.computeBatchHash(
                batchPtr, BatchHeaderV1Codec.BATCH_HEADER_FIXED_LENGTH + _skippedL1MessageBitmap.length
            );
        } else {
        ...
        _checkSkippedL1MessageBitmap(
            _totalL1MessagesPoppedOverall, _totalL1MessagesPoppedInBatch, _skippedL1MessageBitmap, false
        );
        ...
        whenNotPaused
    {
        ...
        (bytes32 _dataHash, uint256 _totalL1MessagesPoppedInBatch) =
            _commitChunksV1(_totalL1MessagesPoppedOverall, _chunks, _skippedL1MessageBitmap);
        unchecked {
        ...
        // verify skippedL1MessageBitmap
        _checkSkippedL1MessageBitmap(
            _totalL1MessagesPoppedOverall, _totalL1MessagesPoppedInBatch, _skippedL1MessageBitmap, true
        );
        BatchHeaderV0Codec.storeL1MessagePopped(batchPtr, _totalL1MessagesPoppedInBatch);
        BatchHeaderV0Codec.storeTotalL1MessagePopped(batchPtr, _totalL1MessagesPoppedOverall);
        BatchHeaderV0Codec.storeDataHash(batchPtr, _dataHash);
        ...
        bytes32 _batchHash = BatchHeaderV0Codec.computeBatchHash(batchPtr, BatchHeaderV3Codec.BATCH_HEADER_FIXED_LENGTH);
    ...
    function revertBatch(bytes calldata _firstBatchHeader, bytes calldata _lastBatchHeader) external onlyOwner {
        (uint256 firstBatchPtr,, uint256 _firstBatchIndex, uint256 _totalL1MessagesPoppedOverallFirstBatch) =
...
//        _afterFinalizeBatch(_totalL1MessagesPoppedOverall, _batchIndex, _batchHash, _postStateRoot, _withdrawRoot);
        ...
        whenNotPaused
    {
        ...
        (uint256 batchPtr, bytes32 _batchHash, uint256 _batchIndex, uint256 _totalL1MessagesPoppedOverall) =
...
//        _finalizePoppedL1Messages(_totalL1MessagesPoppedOverall);
    ...
    function _checkBlobVersionedHash(bytes32 _blobVersionedHash, bytes calldata _blobDataProof) internal view {
        ...
        (bool success, bytes memory data) =
            POINT_EVALUATION_PRECOMPILE_ADDR.staticcall(abi.encodePacked(_blobVersionedHash, _blobDataProof));
        ...
        private
    {
        ...
        if (_doPopMessage) {
            _popL1MessagesCalldata(
                _skippedL1MessageBitmap, _totalL1MessagesPoppedOverall, _totalL1MessagesPoppedInBatch
            );
        }
    }
    ...
    /// @return _totalL1MessagesPoppedInBatch The total number of L1 messages popped in this batch, including skipped
    /// one.
        ...
        returns (bytes32 _batchDataHash, uint256 _totalL1MessagesPoppedInBatch)
    {
        ...
        for (uint256 i = 0; i < _chunksLength; i++) {
            ...
            (_chunkDataHash, _totalNumL1MessagesInChunk) = _commitChunkV0(
                _chunks[i], _totalL1MessagesPoppedInBatch, _totalL1MessagesPoppedOverall, _skippedL1MessageBitmap
            );
            unchecked {
    ...
    /// @param _totalL1MessagesPoppedInBatch The total number of L1 messages popped in the current batch before this
    /// chunk.
    /// @param _totalL1MessagesPoppedOverall The total number of L1 messages popped in all batches including the current
    /// batch, before this chunk.
    /// @param _skippedL1MessageBitmap The bitmap indicates whether each L1 message is skipped or not.
    ...
    /// @param _chunk The encoded chunk to commit.
    /// @param _totalL1MessagesPoppedInBatch The total number of L1 messages popped in current batch.
    /// @param _totalL1MessagesPoppedOverall The total number of L1 messages popped in all batches including current
    /// batch.
    /// @param _skippedL1MessageBitmap The bitmap indicates whether each L1 message is skipped or not.
    /// @return _dataHash The computed data hash for this chunk.
    /// @return _totalNumL1MessagesInChunk The total number of L1 message popped in current chunk
    ...
    /// @param totalL1MessagesPoppedOverall The total number of L1 messages popped in all batches including current
    /// batch.
        ...
        internal
    {
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < totalL1MessagesPoppedInBatch; i += 256) {
                ...
                assembly {
                    switch isCalldata
                    case 1 { bitmap := calldataload(bitmapPtr) }
                    default { bitmap := mload(bitmapPtr) }
