...
contract DeployHyperT1MessengerProxy is Script, DeploymentUtils {
    ...
    function run() external {
        vm.createSelectFork(vm.rpcUrl("hyperevm_sepolia"));
    ...
    function deployPlaceHolder() internal {
        ...
        logAddress("HYPER_PROXY_IMPLEMENTATION_PLACEHOLDER_ADDR", address(placeholder));
    }
    ...
    function deployT1MessengerProxy() internal {
        ...
        logAddress("HYPER_T1_MESSENGER_PROXY_ADDR", address(proxy));
    }
}

contract DeployHyperT1MessengerImplAndInit is Script, DeploymentUtils {
    uint256 private deployerPrivateKey = vm.envUint("DEPLOYER_PRIVATE_KEY");
    address private HYPER_T1_PROXY_ADMIN_ADDR = vm.envAddress("HYPER_T1_PROXY_ADMIN_ADDR");
    address private HYPER_T1_MESSENGER_PROXY_ADDR = vm.envAddress("HYPER_T1_MESSENGER_PROXY_ADDR");
    ...
    function run() external {
        ...
        proxy = TransparentUpgradeableProxy(payable(HYPER_T1_MESSENGER_PROXY_ADDR));
        deployMessageQueue();
    ...
    function deployMessageQueue() internal {
        ...
        logAddress("HYPER_T1_MESSAGE_QUEUE_ADDR", address(queue));
    }

    function deployT1MessengerImpl() internal {
        impl = new L2T1Messenger(BASE_T1_MESSENGER_PROXY_ADDR, address(queue));

        logAddress("HYPER_T1_MESSENGER_IMPLEMENTATION_ADDR", address(impl));
    }
...
contract DeployHyperT1MessengerOwnerAndTransferOwnership is Script, DeploymentUtils {
    uint256 private deployerPrivateKey = vm.envUint("DEPLOYER_PRIVATE_KEY");
    address private HYPER_T1_PROXY_ADMIN_ADDR = vm.envAddress("HYPER_T1_PROXY_ADMIN_ADDR");
    ...
    function deployT1Owner() internal {
        owner = new T1Owner();
        logAddress("HYPER_T1_OWNER_ADDR", address(owner));
    }
