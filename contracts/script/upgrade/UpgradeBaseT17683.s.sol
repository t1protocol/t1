...
contract UpgradeBaseT1ERC7683 is DeploymentUtils {
    ...
    address internal BASE_T1_X_CHAIN_READ_PROXY_ADDR = vm.envOr("BASE_T1_X_CHAIN_READ_PROXY_ADDR", address(0));
    ...
    function upgrade() external {
        logStart("Upgrade T1ERC7683 on Base");
        ...
        proxyAdmin.upgrade(ITransparentUpgradeableProxy(BASE_T1_7683_PROXY_ADDR), address(newImpl));

        vm.stopBroadcast();

        logAddress("BASE_T1_PULL_BASED_7683_IMPLEMENTATION_ADDR", address(newImpl));
        logAddress("BASE_T1_PULL_BASED_7683_PROXY_ADDR", BASE_T1_7683_PROXY_ADDR);
        logEnd("Upgrade T1ERC7683 on Base");
    }
}
