...
contract Setup7683BotsLiquidity is Script, DeploymentUtils {
    ...
    function fundBots() {
        ...
        address payable fillBotAddr = payable(vm.addr(BASE_SEPOLIA_FILL_BOT_PRIVATE_KEY));
        ...
        // grant WETH to bot
        WrappedEther(BASE_SEPOLIA_WETH_ADDR).transfer(fillBotAddr, wethAmount);
        // grant USDT to bot
        T1StandardERC20(BASE_SEPOLIA_USDT_ADDR).transfer(fillBotAddr, usdtAmount); // 2M USDT
    ...
    function approveContracts() {
        ...
        T1StandardERC20(BASE_SEPOLIA_USDT_ADDR).approve(PULL_BASED_7683_PROXY_ADDR, type(uint256).max - 1);
        // approve L1 7683 Escrow to transfer WETH in the bot's name
        WrappedEther(BASE_SEPOLIA_WETH_ADDR).approve(PULL_BASED_7683_PROXY_ADDR, type(uint256).max - 1);
